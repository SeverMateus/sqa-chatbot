...
public class ApiResource {
  ...
  /**
   * Method in charge of receiving the POST request for the endpoint.
   * @param req The request sent in JSON format
   * @return The response to that request in JSON format
   */
  @POST
  @Produces(MediaType.APPLICATION_JSON)
  ...
  private String parseJson(String req) {
    ...
    switch (action) {
      case "support.delivery":
        resp = DataAccess.getStatus(
            json.getJSONObject("result").getJSONObject("parameters").getString("order-number"));
        break;
      case "sales.price":
        resp = DataAccess.getPrice(
            json.getJSONObject("result").getJSONObject("parameters").getString("product-code"));
        break;
      default:
